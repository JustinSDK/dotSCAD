module voronoi_square(size, cell_w, seed, spacing = 1, r = 0, delta = 0, chamfer = false, region_type = "square") {
    ...
    cell_nbrs_lt = [for(cy = [0:cell_w:size[1]]) 
        for(cx = [0:cell_w:size[0]])
        let(
            nbrs = _neighbors(
                [floor(cx / cell_w), floor(cy / cell_w)],
                sd, 
                cell_w
            ),
            p = nbrs[4],
